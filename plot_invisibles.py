...
from common_functions import original_tau, original_tau_cart
...
def plot(scaled_Y, X, Y, B, L, channel, out_folder=''):
    ...
    tau_1_orig_cart = original_tau_cart(0, 1, 2, 3, 13, 14, 15, 16, X, scaled_Y)
    gentau_1_orig_cart = original_tau_cart(0, 1, 2, 3, 13, 14, 15, 16, X, Y)

    tau_2_orig_cart = original_tau_cart(4, 5, 6, 7, 17, 18, 19, 20, X, scaled_Y)
    gentau_2_orig_cart = original_tau_cart(4, 5, 6, 7, 17, 18, 19, 20, X, Y)
...
#            ax.text(0.7, 0.2, str(np.mean(diff_physfourvectors[:,a]))[0:4] +" GeV", fontsize=10, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
    ...
    for a in range(regressed_fourvectors.shape[1]):
        fig = plt.figure(figsize=(3,3))
        ax = fig.add_subplot(111)
        ranges = [[0,500], None, None, None]
        titles = [ r'$E$ (GeV)', r'$p_x$',r'$p_y$',r'$p_z$ (GeV)',]
        n, bins, patches = plt.hist(target_fourvectors[:,a], 50, color='green', range=ranges[a], histtype='step', label='target')
        n, bins, patches = plt.hist(regressed_fourvectors[:,a], 50, color='red', range=ranges[a], histtype='step', label='regressed')
        n, bins, patches = plt.hist(vis_fourvectors[:,a], 50, color='orange', range=ranges[a], histtype='step', label='visible', linestyle='dotted')
        print titles[a], " & ", "{:2.2f}".format(np.mean(diff_fourvectors[:,a])), " & ", "{:2.2f}".format(np.std(diff_fourvectors[:,a])), "\\\\"
#        print "phys diffvector mean ", a, np.mean(diff_physfourvectors[:,a]), " stddev " , np.std(diff_physfourvectors[:,a])
#        if a == 0:
#            ax.text(0.7, 0.5, r'$\sigma(p_T^{true}, p_T^{reg.})$ = ', fontsize=12, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
#            ax.text(0.7, 0.4, str(np.std(diff_physfourvectors[:,a]))[0:4] +" GeV", fontsize=10, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
...
#            ax.text(0.7, 0.2, str(np.mean(diff_physfourvectors[:,a]))[0:4] +" GeV", fontsize=10, horizontalalignment='center', verticalalignment='center', transform = ax.transAxes)
        ...
        plt.legend(loc='best')
        plt.tight_layout()
        plt.savefig(os.path.join(out_folder, "cart-target-regressed"+str(a)+".pdf"))
        plt.savefig(os.path.join(out_folder, "cart-target-regressed"+str(a)+".png"))
        plt.close()
    # original tau cartesian
...
#            arange = [-1,6]
        ...
        n, bins, patches = plt.hist(tau_1_orig_cart[:,a], 50, normed=1, color="gray", histtype='step', range = arange, label='regressed tau1')
        n, bins, patches = plt.hist(tau_2_orig_cart[:,a], 50, normed=1, color="green", histtype='step', range = arange, label='regressed tau2')
        n, bins, patches = plt.hist(gentau_1_orig_cart[:,a], 50, normed=1, color="black", histtype='step', range = arange, label='target tau1')
        n, bins, patches = plt.hist(gentau_2_orig_cart[:,a], 50, normed=1, color="orange", histtype='step', range = arange, label='target tau2')
        ax.set_xlabel(labels[a])
        ax.set_ylabel("arb. units")
        ax.set_title("Tau mass (" + channels[channel] + ")")
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(out_folder, "original-tau-cart"+str(a)+".pdf"))
        plt.savefig(os.path.join(out_folder, "original-tau-cart"+str(a)+".png"))
        plt.close()
	# deltaphi
    fig = plt.figure(figsize=(5,5))
    ax = fig.add_subplot(111)
    ...
    n, bins, patches = plt.hist(np.arctan2(tau_1_orig_cart[:,2], tau_1_orig_cart[:,1]) - np.arctan2(gentau_1_orig_cart[:,2], gentau_1_orig_cart[:,1]), 50, normed=1, color="gray", histtype='step', range = arange, label='regressed tau1')
    n, bins, patches = plt.hist(np.arctan2(tau_2_orig_cart[:,2], tau_2_orig_cart[:,1]) - np.arctan2(gentau_2_orig_cart[:,2], gentau_2_orig_cart[:,1]), 50, normed=1, color="green", histtype='step', range = arange, label='regressed tau2')
#    n, bins, patches = plt.hist(tau_2_orig_cart[:,a], 50, normed=1, color="green", histtype='step', range = arange, label='regressed tau2')
#    n, bins, patches = plt.hist(gentau_1_orig_cart[:,a], 50, normed=1, color="black", histtype='step', range = arange, label='target tau1')
#    n, bins, patches = plt.hist(gentau_2_orig_cart[:,a], 50, normed=1, color="orange", histtype='step', range = arange, label='target tau2')
    ax.set_xlabel("phi")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_folder, "original-tau-phi.pdf"))
    plt.savefig(os.path.join(out_folder, "original-tau-phi.png"))
    plt.close()
     # original tau phys
