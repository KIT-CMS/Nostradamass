...
#        eta = -2.7 + np.random.rand() * 2. *2.7
#        eta =  np.random.normal(0,2.)
        eta = 3
        while(abs(eta) > 2.7):
            eta = np.random.normal(0,1.5)
        #eta = 0
        pt = np.random.exponential(scale=50)+15
#        pt = -1
 #       while(pt<10):
  #          pt = np.absolute(np.random.normal(20,50))
        if particle == 0:
            m = 0.005 # electron
        elif particle == 1:
            m = 0.1 # muon
            ...
            if piZeros == 0:
                ...
                m = np.max([0.1, np.random.normal(1.,0.5)])
...
def hard_neutrino(lepton):
    while(True):
        phi = np.random.rand() * np.pi * 2 # phi from 0 to 2 pi
        eta = np.random.normal(0,1) 
        #eta = 0
        #pt = np.absolute(np.random.normal(0,30)/100*lepton.pt)
        pt = np.random.rand() * lepton.pt
        m = 0.0 # massless
        return FourMomentum(m, pt, eta, phi, False)
...
def fake_met():
    while(True):
        phi = np.random.rand() * np.pi * 2 # phi from 0 to 2 pi
        eta = 0 
        pt = np.random.exponential(scale=10)
        pt = 0 
        m = 0.0 # massless
        yield FourMomentum(m, pt, eta, phi, False)
...
# throw toys according to expected four-vectors
    ...
    neutrino_sum += hard_neutrino(lepton_1)
    if lepton1<2:
        neutrino_sum += hard_neutrino(lepton_1)
    neutrino_sum += hard_neutrino(lepton_2)
    if lepton2<2:
        neutrino_sum += hard_neutrino(lepton_2)
    ...
    x = np.array([lepton_1.e, lepton_1.px, lepton_1.py, lepton_1.pz, lepton_2.e, lepton_2.px, lepton_2.py, lepton_2.pz, met.px, met.py, met.pt2(),
                  ...
                  #lepton_1.e**2, lepton_1.px**2, lepton_1.py**2, lepton_1.pz**2, lepton_2.e**2, lepton_2.px**2, lepton_2.py**2, lepton_2.pz**2, met.px**2, met.py**2, met.pt2()**2])
    X[i,:] = x
...
#Y = scalerMinMax.fit_transform(Y)
...
# model def
model = Sequential()
model.add(Dense(15, activation='tanh', input_shape=(X.shape[1],)))
